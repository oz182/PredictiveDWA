...
class SubprocessSimulationGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Predictive DWA Simulation (Subprocess)")
...
'''
            ...
            script_path = "temp_simulation.py"
            with open(script_path, 'w') as f:
                f.write(script_content)
            
            # Start the simulation process
            self.simulation_process = subprocess.Popen([sys.executable, script_path])
            ...
            self.status_label.config(text="Failed to start simulation")
...
def main():
    root = tk.Tk()
    app = SubprocessSimulationGUI(root)
