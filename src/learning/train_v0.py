...
def compute_reward(sim, progress_prev_dist: float, offset: float = 0.0, door_dy: Optional[float] = None) -> Tuple[float, float, dict]:
    ...
    # desired_dist_from_door = 0.55  # meters
    # if door_dy is None:
    #     door_y = float(sim.robot.door_position[1])
    #     door_dy = float(door_y - robot_pos[1])
    # if door_dy > desired_dist_from_door:
    #     # Linear penalty when robot is too far towards the door side
    #     reward -= (door_dy - desired_dist_from_door)/20.0
